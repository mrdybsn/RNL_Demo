...
const EditGenderForm: FC<EditGenderFormProps> = ({onGenderUpdated}) => {
    ...
    const handleGetGender = async (genderId: string | number) => {
        try {
            ...
            const res = await GenderServices.getGender(genderId);
    ...
    const handleUpdateGender = async (e: FormEvent) => {
        try{
            ...
            const res = await GenderServices.updateGender(gender_id!, {gender})
            ...
            if(res.status === 200) {
                ...
                console.error('Unexpected status error occurred during updating gender: ', res.status)
            }
        }catch(error: any) {
            if(error.response && error.response.status === 422) {
                setErrors(error.response.data.errors)
            }else{
                console.error('Unexpected server error occurred during updating gender:', error)
            }
        } finally {
